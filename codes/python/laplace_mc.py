...
def plot_initialize(i0,j0):
    ...
    plt.clf()
    plt.axes([.1,.3,.8,.6]) # leave room for printing messages
    x = np.linspace(ax,bx,nx+2)
    y = np.linspace(ay,by,ny+2)
    X,Y = np.meshgrid(x,y)  # turn into 2d arrays
    plt.plot(X,Y,'k')
    plt.plot(X.T,Y.T,'k')

    plt.plot([ax+i0*dx],[ay+j0*dy],'ro')
    plt.axis('scaled')
    plt.title("Initial point for random walk")
    plt.ion()
    plt.draw()
    plt.pause(0.001)
    plt.show()
...
def plot_ub(xb,yb,ub):
    ...
    """
    plt.plot([xb], [yb], 'ro')
    plt.text(ax, ay-0.2, 'Hit boundary where u = %9.6f' % ub, fontsize=20)
    plt.draw()
    plt.pause(0.001)
    time.sleep(2)
...
def plot_step(iold, jold, i, j, istep):
    ...
    plt.plot([ax+iold*dx, ax+i*dx], [ay+jold*dy, ay+j*dy], 'r',linewidth=2)
    plt.plot([ax+i*dx], [ay+j*dy], 'ro')
    plt.title("After %6i random steps" % (istep+1))
    plt.draw()
    plt.pause(0.001)

    time.sleep(0.05)
    # redraw last segment and point in blue:
    plt.plot([ax+iold*dx, ax+i*dx], [ay+jold*dy, ay+j*dy], 'b',linewidth=2)
    plt.plot([ax+i*dx], [ay+j*dy], 'bo')
    plt.draw()
    plt.pause(0.001)
...
def random_walk(i0, j0, max_steps):
    ...
    for istep in range(max_steps):
        ...
        if istep==(max_steps-1):
            ...
            if plot_walk:
                plt.text(0, -0.2, "Did not hit boundary after %i steps" \
...
if __name__ == "__main__":
    ...
    ans = input("Enter seed for random generator or <return> .... ")
    if ans == "":
